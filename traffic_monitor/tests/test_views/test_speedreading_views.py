import pytest
from traffic_monitor.models import SpeedReading


@pytest.mark.django_db
def test_get_speed_reading_list(api_client, sample_speed_readings):

    response_read = api_client.get("/api/speed_readings/", format="json")
...
def test_create_speed_reading(api_client, super_user, sample_road_segment):
    ...
    response_read = api_client.get(f"/api/speed_readings/{new_speed_reading_id}/")
...
@pytest.mark.django_db
def test_create_speed_reading_without_admin_credentials(
    api_client, user, sample_road_segment
):
...
def test_get_speed_reading_detail(api_client, sample_road_segment):
    ...
    valid_response_read = api_client.get(
        f"/api/speed_readings/{speed_reading.id}/", format="json"
    )

    assert valid_response_read.status_code == 200
    assert valid_response_read.data["speed"] == 25.0

    invalid_response_read = api_client.get(f"/api/speed_readings/11111/", format="json")
...
def test_update_speed_reading(
    ...
    response_update = api_client.put(
        ...
        data={"speed": 99.9, "road_segment": sample_road_segment.id},
        format="json",
    )
    ...
    response_read = api_client.get(
        f"/api/speed_readings/{sample_speed_reading.id}/", format="json"
    )
...
@pytest.mark.django_db
def test_incomplete_update_speed_reading(
    api_client, super_user, sample_speed_reading, sample_road_segment
):
...
@pytest.mark.django_db
def test_partial_update_speed_reading_without_credenticals(
    api_client, sample_speed_reading
):
