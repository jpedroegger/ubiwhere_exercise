...
def test_road_segment_serializer_duplicate():
    RoadSegment.objects.create(
        coordinate=LineString([(1.0, 2.0), (3.0, 4.0)]),
        road_length=1000.0
    )
    duplicate = {
        "coordinate": {
            "type": "LineString",
            "coordinates": [(3.0, 4.0), (1.0, 2.0)]
        },
        "road_length": 900.0
    }
...
@pytest.mark.django_db
def test_road_segment_returns_correct_speed_records_count(sample_road_segment, sample_speed_readings):
